...
def process_input():
	"""Returns a 20x20 array of numbers"""
	...
	rows =  split(raw_input, sep='\n')
	table=[]
	for row in rows:
		table.append(split(row))
	return table

def find_max(table):
	"""Returns the maximum product of 4 adjacent numbers"""
	high = 0
	for i in range(20):
		for j in range(20):
			high = max(downleft(i,j,table),
				   down(i,j,table),
				   downright(i,j,table),
				   right(i,j,table),
				   high)
...
def downleft(i,j,table):
	"""Returns the product to down-left diagonal"""
	product = 1
	for num in range(4):
...
def down(i,j,table):
	"""Return the product of going down"""
	product = 1
	for num in range(4):
		if i+num>19: product *=1
		else: product *= int(table[i+num][j])
	return product
